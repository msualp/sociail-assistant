#!/usr/bin/env python3
"""
Generate v3 r3.1-r2 MagSafe form-factor printable STL assets.
...
OUT_DIR = Path("prototype/v3/r3-1-r2")
...
SNAP_ENGAGE = 0.82
SNAP_TAB_THICK = 1.45
SNAP_SLOT_DEPTH = 1.65
SNAP_NOTCH_EXTRA = 0.62

# Apple-leaning ergonomic contour set: less boxy, smoother in-hand transitions.
SIDE_SCALLOP_DEPTH = 2.55
SIDE_SCALLOP_RADIUS = 52.0
SHOULDER_SCALLOP_DEPTH = 1.80
SHOULDER_SCALLOP_RADIUS = 56.0
CORNER_BLEND_DEPTH = 2.05
CORNER_BLEND_RADIUS = 46.0
DRAFT_SIDE_DEPTH = 1.75
DRAFT_SIDE_RADIUS = 34.0
DRAFT_SIDE_Z = HEIGHT * 0.58
DRAFT_SHOULDER_DEPTH = 1.10
DRAFT_SHOULDER_RADIUS = 32.0
DRAFT_SHOULDER_Z = HEIGHT * 0.60
GLOBAL_DRAFT_BOTTOM_SCALE = 1.06
GLOBAL_DRAFT_TOP_SCALE = 0.86
GLOBAL_DRAFT_BANDS = 8
...
def apply_global_draft(body: mr.Mesh) -> mr.Mesh:
    ...
    for i in range(bands):
        ...
        t = ((z0 + z1) * 0.5) / HEIGHT
        s = GLOBAL_DRAFT_BOTTOM_SCALE + (GLOBAL_DRAFT_TOP_SCALE - GLOBAL_DRAFT_BOTTOM_SCALE) * t
...
def main() -> None:
    ...
    (OUT_DIR / "README.md").write_text(
        """# v3 R3.1-r2 Holistic Exterior Redesign (MagSafe)

Generated print assets for the v3.1-r2 holistic redesign: stronger all-around corner rounding and full-body trapezoidal draft shaping.

## What Changed vs R2
- Larger perimeter edge rounds and softened crown transitions
- Added subtle side/shoulder contouring for improved in-hand comfort
- Added MagSafe pocket alignment bossing + anti-rotation key
- Upgraded seam to 6 snap points for stronger, more even retention
- Tuned clearances for repeatable assembly/disassembly on FDM prints
- Geometry tuned to print cleanly in recommended orientations with minimal supports
