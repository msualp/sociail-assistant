...
```
    ...
    Rounded-rectangle profile: 35mm W x 14mm H with smooth top/bottom edge fillets (target 4mm).
```
...
| Front profile corner radius | 4.0 mm target | Controlled by edge fillets |
...
**Origin**: Centered in XY on the bottom plane (X=0, Y=0, Z=0)
